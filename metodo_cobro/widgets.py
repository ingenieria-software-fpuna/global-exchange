...
class MonedaSelectorWidget(forms.SelectMultiple):
    """
    Widget personalizado para selección de monedas con tabla interactiva,
    búsqueda y mejor UX.
    """
    
    def __init__(self, attrs=None):
        default_attrs = {
            'class': 'moneda-selector',
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        widget_id = f"moneda-selector-{name.replace('[', '-').replace(']', '')}"
        
        # Obtener monedas seleccionadas
        selected_ids = []
        if value:
        ...
        # Obtener monedas activas para la tabla
        monedas = Moneda.objects.filter(
            es_activa=True
        ).values('id', 'nombre', 'codigo', 'simbolo').order_by('codigo')
        
        # Generar HTML del widget personalizado
        widget_html = f"""
        {original_select}
        <div class="moneda-selector-widget" id="{widget_id}">
            <!-- Buscador -->
            <div class="mb-3">
                <div class="input-group">
                    ...
                    <input type="text" 
                           id="{widget_id}-search" 
                           class="form-control" 
                           placeholder="Buscar monedas por código o nombre...">
                </div>
            </div>
            ...
            <div class="mb-3">
                <div class="alert alert-info d-flex justify-content-between align-items-center">
                    <span>
                        ...
                        Monedas seleccionadas: <strong id="{widget_id}-count">{len(selected_ids)}</strong>
                    </span>
                    <button type="button" 
            ...
            <!-- Alerta de advertencia cuando no hay monedas seleccionadas -->
            <div id="{widget_id}-warning" class="alert alert-warning" style="{'display: none;' if selected_ids else ''}">
                <i class="fas fa-exclamation-triangle me-2"></i>
                <strong>Advertencia:</strong> Debe seleccionar al menos una moneda para este método de cobro.
            </div>
        ...
        # Agregar filas de monedas
        for moneda in monedas:
            is_selected = str(moneda['id']) in selected_ids
            selected_class = 'table-primary' if is_selected else ''
            checked = 'checked' if is_selected else ''
            
            widget_html += f"""
                        <tr class="moneda-row {selected_class}" data-moneda-id="{moneda['id']}" data-widget="{widget_id}">
                            <td>
                                <input type="checkbox" 
                                       class="form-check-input moneda-checkbox" 
                                       value="{moneda['id']}" 
                                       data-widget="{widget_id}"
                                       {checked}>
                            </td>
                            <td class="moneda-codigo">{moneda['codigo']}</td>
                            <td class="moneda-nombre">{moneda['nombre']}</td>
                            <td class="moneda-simbolo">{moneda['simbolo']}</td>
                        </tr>
            """
        ...
        widget_html += f"""
            ...
            <div id="{widget_id}-no-results" class="text-center py-4" style="display: none;">
                ...
                <p class="text-muted">No se encontraron monedas con ese criterio de búsqueda.</p>
            </div>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            ...
            var monedasTable = document.getElementById(widgetId + '-table');
            ...
            if (!originalSelect) {{
                originalSelect = document.querySelector('#id_' + fieldName) ||
                               ...
                               document.querySelector('select.moneda-selector');
            }}
            ...
            function updateOriginalSelect() {{
                ...
                var checkedBoxes = document.querySelectorAll('input[data-widget="' + widgetId + '"].moneda-checkbox:checked');
                var selectedValues = [];
                ...
                var rows = document.querySelectorAll('tr[data-widget="' + widgetId + '"].moneda-row');
                for (var i = 0; i < rows.length; i++) {{
                    var checkbox = rows[i].querySelector('.moneda-checkbox');
            ...
            function updateUIFast() {{
                var checkedCount = document.querySelectorAll('input[data-widget="' + widgetId + '"].moneda-checkbox:checked').length;
            ...
            function filterMonedas() {{
                ...
                for (var i = 0; i < rows.length; i++) {{
                    var codigo = rows[i].querySelector('.moneda-codigo').textContent.toLowerCase();
                    var nombre = rows[i].querySelector('.moneda-nombre').textContent.toLowerCase();
                    var simbolo = rows[i].querySelector('.moneda-simbolo').textContent.toLowerCase();
                    
                    var matches = codigo.indexOf(searchTerm) >= 0 || 
                                  nombre.indexOf(searchTerm) >= 0 || 
                                  simbolo.indexOf(searchTerm) >= 0;
                ...
                if (noResultsDiv && monedasTable) {{
                    if (visibleCount === 0 && searchTerm) {{
                        noResultsDiv.style.display = 'block';
                        monedasTable.style.display = 'none';
                    }} else {{
                        noResultsDiv.style.display = 'none';
                        monedasTable.style.display = '';
                    }}
                }}
            }}
            
            if (searchInput) {{
                searchInput.addEventListener('input', filterMonedas);
            }}
            
            if (selectAllCheckbox) {{
                selectAllCheckbox.addEventListener('change', function() {{
                    var visibleCheckboxes = document.querySelectorAll('tr[data-widget="' + widgetId + '"].moneda-row:not([style*="display: none"]) .moneda-checkbox');
            ...
            if (clearSelectionBtn) {{
                clearSelectionBtn.addEventListener('click', function() {{
                    var checkboxes = document.querySelectorAll('input[data-widget="' + widgetId + '"].moneda-checkbox');
            ...
            document.addEventListener('change', function(e) {{
                if (e.target && e.target.classList && e.target.classList.contains('moneda-checkbox') && e.target.dataset.widget === widgetId) {{
                    updateUIFast(); // Actualización inmediata del contador y alerta
                    
                    // Actualización del "seleccionar todo" checkbox
                    var allVisible = document.querySelectorAll('tr[data-widget="' + widgetId + '"].moneda-row:not([style*="display: none"]) .moneda-checkbox');
                    var allChecked = true;
            ...
            document.addEventListener('click', function(e) {{
                ...
                var row = e.target.closest('tr[data-widget="' + widgetId + '"].moneda-row');
                if (row && e.target.classList && !e.target.classList.contains('moneda-checkbox') && !e.target.closest('input')) {{
                    var checkbox = row.querySelector('.moneda-checkbox');
                    if (checkbox) {{
